...
## 这次做了什么（首页文案与流程演示）
- 首页主标题/副标题已更新为 AI 翻译与 RPG Maker MV/MZ 专注说明
- Hero 右侧改为 MTool 风格两步流程图，使用 `gamelogo/1.png`（游戏文件夹）与 `gamelogo/2.png`（GameLingo 界面）真截图
- 流程支持自动轮播与点击步骤切换；中英文切换文案在 `scripts/site.js` 的 `flowStep1Title` / `flowStep2Title` 等字段

## 历史：移动端优化
- 首页新增更细的手机断点（`640px / 420px`），避免手机端溢出、挤压、错位
- `价格方案` 由桌面端三列展示，手机端自动变成单列（不再排版乱）
- Hero 右侧演示区域在手机端改为更稳定的单列展示
